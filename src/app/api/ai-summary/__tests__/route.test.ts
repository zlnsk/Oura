 ...
 * (next-auth, fetch) to test the POST handler behavior.
 */
...
// Mock NextRequest/NextResponse since we're in a Node test environment
...
// We'll test the rate limit logic and request validation independently
...
describe("AI Summary API - Rate Limiter Logic", () => {
  ...
  it("resets after the reset time passes", () => {
    // Set an entry with a past reset time
...
describe("AI Summary API - Request Validation", () => {
  it("rejects requests without session (unauthorized)", async () => {
    mockGetServerSession.mockResolvedValueOnce(null);

    // Simulate what the route does: check session first
...
describe("AI Summary API - Prompt Building", () => {
  it("handles page types correctly", () => {
    ...
    // Just verify the page types are recognized as valid strings
  ...
  it("defaults to dashboard when page is not specified", () => {
    ...
    const pageType = (body.page as string) || "dashboard";
